...
# --- SI QUIERES APLICAR LO MISMO AL SEGUNDO BLOQUE DE CÓDIGO (COMENTADO), DESCOMENTALO Y APLICA ESTOS CAMBIOS ---
"""
# import pandas as pd
# import matplotlib.pyplot as plt
...
# # ... (resto del código de procesamiento de datos es el mismo)

# # Paleta de colores por período (igual)
# colores_azules = ['lightblue', 'cornflowerblue', 'royalblue', 'navy']
# colores_rosas = ['lightpink', 'hotpink', 'deeppink', 'mediumvioletred', 'darkmagenta']
# colores_por_periodo = {}
# for p in range(2, 6):  # 2 a 5
...
# # --- MODIFICACIONES PARA EL SEGUNDO BLOQUE DE CÓDIGO ---

# # Definir los límites explícitos para el eje k también para este bloque
# k_min_manual = -45
# k_max_manual = -5
...
#     ax.set_title(f"Estabilidad '{est}'")
#     ax.set_xlabel("b")
#     ax.set_ylabel("k", rotation=0, ha='right') # Label vertical
#     ax.legend(fontsize='small')
#     ax.grid(True)
#     ax.set_ylim(k_min_manual, k_max_manual) # Límites de k iguales

# # Último subplot: número de órbitas por (b,k)
# ax = axes[1][1]
# conteos = df.groupby(['b', 'k']).size().reset_index(name='cantidad')
# sc = ax.scatter(conteos['b'], conteos['k'], c=conteos['cantidad'], cmap='Reds', s=3)
# ax.set_title("Número de órbitas distintas por (b, k)")
# ax.set_xlabel("b")
# ax.set_ylabel("k", rotation=0, ha='right') # Label vertical
# ax.grid(True)
# fig.colorbar(sc, ax=ax, label='Número de órbitas')
# ax.set_ylim(k_min_manual, k_max_manual) # Límites de k iguales también para este plot
